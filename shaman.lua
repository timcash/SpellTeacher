...
local function HealSelf()
    ...
    if NS.getHealthPercent("player") < 80 then
        if stacks > 4 then
            if NS.dbGet("player_last_spell_id") ~= id then
                return true
            end 
...
local function Kick()
    local id = windShearId
    if NS.canInterrupt("target") then
        if NS.spellReadyIn(id) < 0.1 then
            return true
        end
    end
    return false
...
local function calc1()
    ...
    if Kick() then return windShearId end
...
NS["SHAMAN/Enhancement"] = {}
NS["SHAMAN/Enhancement"].calc1 = calc1
NS["SHAMAN/Enhancement"].calc2 = calc2
NS["SHAMAN/Enhancement"].calc3 = calc3
NS["SHAMAN/Enhancement"].calc4 = calc4